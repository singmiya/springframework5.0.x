 ...
 * this interface.
 *
 * BeanFactory接口的扩展，此扩展由可以枚举所有bean实例的bean工厂实现，而不是根据客户端请求按照名称一一尝试查找bean。
 * 需要实现预加载所有bean定义（例如，基于XML的工厂）的BeanFactory可能需要实现此接口。
 *
 ...
 *
 * 如果这是HierarchicalBeanFactory，返回值将不会将任何BeanFactory层次结构考虑在内，仅会将与当前工厂定义的bean相关联。
 * 也可以使用BeanFactoryUtils帮助类把祖先工厂中的bean考虑在内。
 *
 ...
 *
 * 该接口中的方法仅遵守此工厂中的bean定义。它们将会忽略由其他方式注册的任何单例bean（像ConfigurableBeanFactory的registerSingleton方法），
 * 除了getBeanNamesOfType和getBeansOfType之外，它们也会检查此类手动注册的单例。
 * 当然，BeanFactory的getBean方法也允许透明的访问此类特殊bean。然而，在典型场景下，无论如何，所有的bean都将通过外部bean定义来定义，
 * 因此，大部分应用都不需要担心此类差异。
 *
 ...
 *
 * 注意：除了getBeanDefinitionCount和containsBeanDefinition之外，此接口中的方法不适合频繁调用。其实现（运行速度）可能比较慢。
 *
 * @author Rod Johnson
...
public interface ListableBeanFactory extends BeanFactory {
	 ...
	 *
	 * 不考虑此工厂涉及到的任何层次结构，并忽略通过除bean定义以外的其他方式注册的任何单例bean。
	 *
	 ...
	 * other means than bean definitions.
	 *
	 * 不考虑此工厂涉及到的任何层次结构。并忽略通过除bean定义以外的其他方式定义的任何单例bean。
	 *
	 * @return the number of beans defined in the factory
	 */
	 ...
	 * other means than bean definitions.
	 *
	 * 不考虑此工厂涉及到的任何层次结构，并忽略通过除bean定义之外的其他方式注册的任何单例bean。
	 *
	 * @return the names of all beans defined in this factory,
	 ...
	 *
	 * 注意：此方法仅内省（introspects）顶级bean。它不会检查可能也匹配指定类型的内嵌bean。
	 *
	 ...
	 *
	 * 不会考虑此工厂涉及到的任何层次结构。也可以使用BeanFactoryUtils的beanNamesForTypeIncludingAncestors方法来包含祖先工厂的bean。
	 *
	 * <p>Note: Does <i>not</i> ignore singleton beans that have been registered
	 * by other means than bean definitions.
	 *
	 * 注意：不会忽略痛处出bean定义之外的其他方式注册的单例bean。
	 *
	 * <p>This version of {@code getBeanNamesForType} matches all kinds of beans,
	 * be it singletons, prototypes, or FactoryBeans. In most implementations, the
	 * result will be the same as for {@code getBeanNamesForType(type, true, true)}.
	 *
	 * 此版本的getBeanNamesForType方法匹配所有类型的bean，无论是单例、原型还是FactoryBeans。
	 * 在大部分实现中，结果将会与getBeanNamesForType(type, true, true)返回结果相同。
	 *
	 * <p>Bean names returned by this method should always return bean names <i>in the
	 * order of definition</i> in the backend configuration, as far as possible.
	 *
	 * 对于通过此方法返回的bean名称的顺序，应该总是尽可能的与后台配置的定义的顺序一致。
	 *
	 * @param type the generically typed class or interface to match 要匹配的泛型类或接口
	 * @return the names of beans (or objects created by FactoryBeans) matching
	 * the given object type (including subclasses), or an empty array if none 匹配给定对象类型的bean名称（或由FactoryBeans创建的对象），如果不存在返回空列表
	 * @since 4.2
